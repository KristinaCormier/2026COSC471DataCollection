...
-- [1] J. Helana, “511TableCreationScript.sql,” in Youry, *AlgorithmicTrading*, GitHub. 
-- [Online]. Available: https://github.com/youry/AlgorithmicTrading/blob/main/Data/Import%20Data%20Scripts/511TableCreationScript.sql. 
-- Accessed: Feb. 13, 2026.
...
DECLARE
    ...
    tbl text;
BEGIN
    -- Drop all existing stg_raw tables safely
    -- RAISE NOTICE 'Dropping existing tables...';
    -- FOR sym IN SELECT table_name FROM information_schema.tables WHERE table_schema = 'stg_raw' LOOP
    --     EXECUTE format('DROP TABLE IF EXISTS stg_raw.%I CASCADE;', sym);
    -- END LOOP;
    ...
    FOREACH sym IN ARRAY tickers LOOP
        tbl := lower(sym);
        EXECUTE format('
            CREATE TABLE IF NOT EXISTS stg_raw.%I (
    ...
    RAISE NOTICE 'All stg_raw tables recreated successfully.';
END $$;
